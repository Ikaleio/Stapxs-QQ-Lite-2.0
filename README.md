...
如果你是 Fork 本仓库并使用 Vercel 持续部署，仓库内置了 `.github/workflows/sync-upstream.yml`，会每小时自动同步 `Stapxs/Stapxs-QQ-Lite-2.0` 的默认分支到你的默认分支。只要 Vercel 绑定的是该分支（Production Branch），同步后的新提交会自动触发重新部署。
